...
void main() {
  ...
  test('page 1 synthesizes a chronological thread led by the system line', () async {
    final (messages, meta) = await api.execute(config: const GetMessagesConfig(chatId: 'c1', page: 1));
  ...
  test('two chats produce ids distinguished only by their chatId prefix', () async {
    final (first, _) = await api.execute(config: const GetMessagesConfig(chatId: 'alpha', page: 1));
    final (second, _) = await api.execute(config: const GetMessagesConfig(chatId: 'beta', page: 1));
  ...
  test('page 2 hits the end guard and returns an empty list with total preserved', () async {
    ...
    final (messages, meta) = await api.execute(config: const GetMessagesConfig(chatId: 'c1', page: 2));
