# Спецификация фичи: Инициализация Flutter-проекта NOX (multi-platform skeleton)

**Ветка фичи**: `001-flutter-project-init`

**Создано**: 2026-06-08

**Статус**: Черновик

**Исходные данные**: описание от пользователя: "Инициализировать Flutter-проект NOX, поддерживающий все платформы, кроме web (две мобильные — Android, iOS; три десктопные — Windows, Linux, macOS). Проект должен соответствовать спецификации разработки в `docs/blueprints/mobile`. Объём — только каркас (skeleton), без реальных фич."
...
**Авторитет по «как»:** `docs/blueprints/mobile/` (блюпринт, особенно `11-scaffolding-plan.md`). Эта спека фиксирует **что** и **границы объёма**; конкретные шаги сборки выводятся из блюпринта на этапах plan/tasks.
...
**Почему такой приоритет**: Это заголовочный результат фичи и одновременно главный риск — десктоп выходил за рамки исходного мобильного блюпринта (build/secrets, push, deep-links, native-слои были iOS/Android-специфичны); блюпринт уже расширен на desktop в этом change-set (см. Зависимости). Если каркас не собирается под все пять таргетов и не запускается там, где есть ОС, дальнейшая работа бессмысленна.

**Независимая проверка**: На чистом клоне по документированным шагам собрать приложение под все пять таргетов; запустить app shell на macOS, iOS, Android (появляется адаптивный shell — bottom-bar на мобиле, `NavigationRail` на macOS); для Windows/Linux подтвердить успешную сборку. Web-таргет отсутствует.

**Сценарии приёмки**:

1. **Дано** чистый клон и поднятое по документации окружение, **Когда** разработчик собирает и запускает приложение на Android и iOS, **Тогда** запускается app shell с нижней панелью (`Chats`, центральный `+`, `Settings`) без падений.
2. **Дано** то же окружение на macOS, **Когда** разработчик собирает и запускает приложение на macOS, **Тогда** запускается тот же app shell, но с `NavigationRail` (адаптив по форм-фактору), без падений.
3. **Дано** то же окружение, **Когда** разработчик собирает приложение под Windows и Linux, **Тогда** сборка проходит успешно (compile/build-verify); их запуск проверяется позже на CI-раннерах (tracked follow-up).
4. **Дано** конфигурацию проекта, **Когда** перечисляются доступные платформенные таргеты, **Тогда** их ровно пять (Android, iOS, Windows, Linux, macOS) и web среди них нет.
...
**Почему такой приоритет**: Смысл каркаса — быть готовым основанием. Если старт фичи требует доделок структуры, инициализация не выполнила свою задачу.

**Независимая проверка**: Сверить дерево проекта с блюпринтом (`00-architecture-overview.md`, `11-scaffolding-plan.md`): присутствуют слои-папки `lib/data`, `lib/domain`, `lib/presentation`, `lib/di`, `lib/general`, `lib/design`, `lib/resource`; один `pubspec.yaml`; единый DI-init; сквозной `Item`-verification-слайс блюпринта собран по шаблону и не потребовал реструктуризации каркаса.

**Сценарии приёмки**:

1. **Дано** инициализированный проект, **Когда** разработчик открывает дерево `lib/`, **Тогда** присутствуют все слои-папки блюпринта в составе одного пакета `nox_app` с однонаправленными зависимостями (`presentation → domain ← data`).
2. **Дано** инициализированный проект, **Когда** разработчик добавляет фичу по copy-paste-шаблону из блюпринта, **Тогда** ему не нужно менять структуру каркаса, поднимать DI заново или вводить новые инфраструктурные примитивы.
...
**Почему такой приоритет**: Зелёный gate с первого дня задаёт планку качества и не даёт каркасу деградировать; воспроизводимость убирает «у меня работает».

**Независимая проверка**: На чистом клоне выполнить документированные команды gate; кодоген проходит за один прогон, `flutter analyze` даёт ноль ошибок, baseline-тесты проходят, форматирование стабильно при line length 140.

**Сценарии приёмки**:

1. **Дано** инициализированный проект, **Когда** разработчик прогоняет кодоген одним прогоном `build_runner`, **Тогда** он завершается без ошибок и без конфликтов.
2. **Дано** инициализированный проект, **Когда** разработчик запускает анализ, **Тогда** ошибок ноль (сгенерированные файлы исключены из анализа).
3. **Дано** инициализированный проект, **Когда** разработчик запускает baseline-тесты, **Тогда** они проходят.
...
**Почему такой приоритет**: Верность дизайн-системе — принцип конституции; задать её на уровне каркаса дешевле, чем вычищать хардкод позже. Приоритет ниже, чем у сборки и структуры, потому что каркас запускается и без полной темизации.

**Независимая проверка**: Переключить системную тему — app shell переключается между light и dark; grep по коду каркаса не находит хардкод-цветов/отступов/типографики; splash background остаётся тёмным независимо от темы.

**Сценарии приёмки**:

1. **Дано** запущенный app shell, **Когда** меняется системная тема устройства, **Тогда** приложение переключается между light и dark, используя только токены.
2. **Дано** код каркаса, **Когда** его просматривают на предмет стилей, **Тогда** все цвета, отступы и типографика берутся из токенов, без хардкода.
...
### Ключевые сущности *(структурные элементы проекта)*
...
- Блюпринт `docs/blueprints/mobile/` (особенно `11-scaffolding-plan.md`) — авторитет по «как»; эта спека фиксирует «что» и границы объёма, а конкретные шаги сборки выводятся из блюпринта на этапах plan/tasks.
