...
> **Имя на проводе и имя в хранилище — разные намеренно.** Поле приветствия называется `login_ref`, колонка — `id_digest`. Это то же разделение, которое проект уже держит между `ChatWireEntity` и `ChatEntity`: провод называет **источник и роль** («ссылка, по которой узнаётся вошедший»), хранилище называет **содержимое колонки** («дайджест идентификатора»). Совпадение имён было бы удобнее при чтении, но связало бы схему с полем, которое контракт обещает удалить на этапе 2.
