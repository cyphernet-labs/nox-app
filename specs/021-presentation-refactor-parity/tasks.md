...
- [ ] T006 [P] [US1] R5 — mobile create-chat: guard `_cancel`/`PopScope` по `state.isSubmitting` в `lib/presentation/pages/create_chat_page/create_chat_page.dart` (`_narrow`); widget-тест (при submit leading disabled / pop подавлен)
...
- [ ] T014 [P] [US2] E3 — `AppHairlineDividerWidget` (const) в `lib/presentation/widgets/primitives/`; заменить 11 `Divider(height: border.hairline)` (settings_root/identity_card/notifications/…); все затронутые goldens без churn
- [ ] T015 [P] [US2] E6 — приватный `_SettingsPaneHeader{title,leading?}` в `settings_root_page.dart` (menu/detail headers, delta padding сохранить); desktop settings golden без churn
- [ ] T016 [P] [US2] E7 — attachment-preview helper (`onTap` vs `onRemove`) в `lib/presentation/widgets/chat/app_thread_view_widget.dart` (`_bubble`/`_composerBar`); thread goldens без churn
- [ ] T017 [P] [US2] E8 — `showAdaptiveLightbox` в `lib/presentation/helpers/adaptive_lightbox.dart`; перевести `showFileView` + `openImageViewer`; оба viewer goldens (mobile+desktop) без churn
- [ ] T018 [P] [US2] E9 — тонкий `_ElevatedBannerShell` для `app_notice_strip_widget.dart` + `app_info_banner_widget.dart`; оба widget goldens без churn
- [ ] T019 [P] [US2] E10 — вынести приватный `_QrDesktopViewfinder` из `_wide` в `lib/presentation/pages/qr_scan_page/qr_scan_page.dart`; qr desktop golden без churn
