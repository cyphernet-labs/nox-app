...
## Notes (обновлено после /speckit-clarify, 2026-08-30)

Пять уточнений сняты и разнесены по спеке; чеклист перепроверен против обновлённой версии — все пункты по-прежнему проходят (16/16). Что изменилось:

- Добавлен раздел `## Clarifications` с сессией 2026-08-30 и пятью записями «вопрос → ответ».
- Требования выросли с 15 до 20 (FR-016…FR-020: порционная загрузка, таймаут отправки, автоповтор при «слишком часто», журналирование без тел и имён, авторитет имени с сервера).
- Добавлен критерий SC-007 (объём пространства и глубина истории не задерживают показ) и три edge case.
- Самое существенное следствие уточнений: вскрылось, что **оба** нынешних сида (список чатов и история чата) обходят все страницы до конца. На моках это незаметно, против живого сервера — неограниченная предзагрузка. Требование FR-016 и решение R14 переводят их на чтение сквозь кэш по порции; в задачи добавлен T016a.
