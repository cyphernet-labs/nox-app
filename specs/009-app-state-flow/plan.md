...
| **III. Блюпринт обязателен** | ✅ PASS (с расширением подсистемы) | Строим по `03/04/02/05/06`; spine — задокументированный-но-отложенный паттерн `05 §6.1`, материализуется. **Desktop-native gate**: secure storage — платформенная нативная часть → по Принципу III **расширяем на desktop** (Linux libsecret уже в CI, Windows DPAPI; macOS — см. реконсиляцию ниже), а не stub'аем. Дрейф код↔блюпринт чиним тем же change-set. **Реконсиляция macOS (2026-06-26, одобрено мейнтейнером)**: планировавшийся `keychain-access-groups` entitlement требует подписи с dev-командой (нет в текущей фазе) и ломал `flutter run` — gate закрыт изоляцией keychain'а самой песочницы через legacy keychain (`MacOsOptions(usesDataProtectionKeychain: false)`); вернуть entitlement + data-protection keychain при появлении macOS-подписи. См. T001. |
