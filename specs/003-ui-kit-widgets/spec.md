...
- Q: Механизм исключения dev-галереи из релизных сборок и продуктовой навигации? → A: Отдельная dev-точка входа `main_gallery.dart` — ноль следа в продуктовом `main.dart` и релизных бинарниках. *(Позже отменено владельцем 2026-06-15: галерея открывается из продуктового лаунчера `HomePage`; см. FR-015.)*
...
### User Story 4 — Лаунчер + галерея кита (Priority: P3)

Пользователь открывает приложение и попадает на стартовый лаунчер (`HomePage`); по кнопке «Open UI Kit» открывается экран-каталог (`UiKitPage`), где каждый виджет показан в light и dark, для сверки с референс-галереей (`preview.html`).

**Why this priority**: на текущем этапе (реальных продуктовых фич ещё нет) лаунчер — удобная точка входа в кит для ручного ревью/онбординга; кит полезен и без него (golden-тесты — основная визуальная истина).

**Independent Test**: `fvm flutter run` → на старте лаунчер с кнопкой «Open UI Kit» → тап открывает каталог со всеми виджетами в обоих режимах темы; `AppThemeToggle` переключает light/dark.

**Acceptance Scenarios**:

1. **Given** запуск приложения, **When** оно открылось, **Then** виден лаунчер `HomePage` с кнопкой «Open UI Kit».
2. **Given** лаунчер, **When** тап «Open UI Kit», **Then** открывается `UiKitPage` со всеми виджетами кита (примитивы, composite, shell, state-виджеты, stock-виджеты).
3. **Given** `UiKitPage`, **When** нажат `AppThemeToggle`, **Then** каждый виджет корректно перерисовывается в light и dark.
...
- **Лаунчер `HomePage` + галерея `UiKitPage`** — стартовый экран → кнопка «Open UI Kit» → каталог всех виджетов в light+dark; общий `AppThemeToggle` (FR-015).
