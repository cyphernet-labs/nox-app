...
| `sync` store (NEW) | одна запись `state`: `{since: int}`; `SyncDao {read, write, clean}` |

`MessageDao._sortChrono` → сортировка по `seq` (вторичный ключ — `sentAt`-строка). Фильтрация чата — по-прежнему в Dart (`field_rename: snake`).
