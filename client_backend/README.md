...
Контракт провода — `docs/client-backend/protocol/contract-draft.md` (v0).

**Сервер проверяет, кто подключается** (фаза 032): приветствие обязано нести `device_key` и `signature` над `"nox/challenge/v1:" ‖ challenge`, иначе оно отвергается кодом `unauthenticated`. Человек и устройство появляются только через `pair` — единственную команду, разрешённую до приветствия. Прежняя строка «стадия 1: аутентификации нет» была верна до 032 и с тех пор нет.

**Канал закрыт** (фаза 036): слушатель говорит только TLS 1.3, флага «без TLS» нет, а доверие берётся из отпечатка ключа в ссылке спаривания.

## Требования

- Go 1.27+ (`go version`)
- Для смоука файловой цепочки: `curl`

> ⚠️ **`websocat` больше не годится, и это проверено.** Слушатель говорит только TLS 1.3, а сборка `websocat` из homebrew линкуется с Security.framework и до 1.3 не поднимается: `-k wss://…` умирает с `WebSocket SSL error: bad protocol version`. Даже с клиентом, умеющим 1.3, приветствие из старого смоука ниже отвергается — с фазы 032 оно обязано нести `device_key` и подпись.
>
> **Живой смоук теперь один и он настоящий:** `go run ./cmd/smoke '<claim-ссылка>'`. Он дозванивается по `wss`, **сверяет отпечаток** из ссылки, спаривает два устройства, обменивается сообщением и печатает каждый шаг. Стенд одной командой — `../scripts/demo-stand.sh`.
>
> `curl` в файловом смоуке ниже работает как напечатано, но ему нужен `-k`: сертификат самоподписанный, и **`-k` не проверяет отпечаток вовсе** — он просто отключает проверку. Подлинность машины проверяют приложение и `cmd/smoke`, а не эти команды.
...
> ⚠️ **Не запускается как напечатано** (см. «Требования»): `websocat` не умеет TLS 1.3, а приветствие без `device_key`/подписи отвергается с фазы 032. Раздел оставлен как описание протокола; живая проверка — `go run ./cmd/smoke '<claim-ссылка>'`.
...
> ⚠️ **Кадры, не команды.** Раздел описывает, ЧТО идёт по проводу; отправить это `websocat`-ом больше нельзя (см. «Требования»). Живая проверка — `go run ./cmd/smoke '<claim-ссылка>'`.
