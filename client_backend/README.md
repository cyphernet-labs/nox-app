...
Собственный сервер мессенджера NOX для **одного человека и его устройств** (ревизия 2026-09-10; прежняя формулировка «малый круг ~10 человек» отменена): один WebSocket-канал команд (JSON-конверт, глобальный лог событий `seq`, догон по курсору `since`), REST-поверхность (`GET /health`), встроенный SQLite. Один статический бинарник без CGO.
