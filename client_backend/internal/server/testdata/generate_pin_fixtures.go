...
func run() error {
	...
	// THE CHAIN ATTACK, and it needs no secret of ours: our certificate is
	// public, handed to every client that ever dialled the server. Anyone can
	// present [their own leaf, our certificate] and complete the handshake with
	// their own key. A check made on the TOP of the presented chain - which is
	// what the platform hands a certificate callback - hashes OUR key while the
	// session belongs to THEIRS. The leaf is the only certificate that proves
	// anything.
	hostile := append(
		pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: strangerCert}),
		pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: valid})...,
	)
