...
// A greeting reports ownership too, and reports it about the PERSON. Before
// this the machine's own row was never read on the greeting path at all.
func TestGreetingReportsWhetherThisPersonOwnsTheServer(t *testing.T) {
	s := newStore(t)
	ctx := context.Background()
	claimPerson(t, s, "dev-phone")

	owner, err := s.ResolveIdentity(ctx, "dev-phone", "", 200)
	if err != nil {
		t.Fatalf("ResolveIdentity: %v", err)
	}
	if !owner.Owner {
		t.Fatal("the owner's greeting does not report ownership")
	}

	// And a person who does not own it says so. Unreachable through pairing
	// today - one server holds one person until invite-user (Q15) - so the
	// second person is written directly, which is exactly what feature 034
	// will produce through the wire.
	if _, err := s.write.ExecContext(ctx,
		"INSERT INTO users (user_id, label, created_at) VALUES ('u_guest', 'Guest', 250)"); err != nil {
		t.Fatalf("insert second person: %v", err)
	}
	if _, err := s.write.ExecContext(ctx,
		"INSERT INTO devices (device_key, user_id, platform, created_at, last_seen_at) VALUES ('dev-guest', 'u_guest', 'test', 250, 250)"); err != nil {
		t.Fatalf("insert guest device: %v", err)
	}
	guest, err := s.ResolveIdentity(ctx, "dev-guest", "", 260)
	if err != nil {
		t.Fatalf("ResolveIdentity guest: %v", err)
	}
	if guest.Owner {
		t.Fatal("somebody who never claimed the server reports owning it")
	}
}

// Ownership follows the person, so it must not be inferred from being the
// oldest row. The guest is created FIRST here and still owns nothing.
func TestOwnershipIsNotInferredFromRowOrder(t *testing.T) {
	s := newStore(t)
	ctx := context.Background()
	if _, err := s.EnsureServerIdentity(ctx); err != nil {
		t.Fatalf("EnsureServerIdentity: %v", err)
	...
	if _, err := s.write.ExecContext(ctx,
		"INSERT INTO users (user_id, label, created_at) VALUES ('u_early', 'Early', 1)"); err != nil {
		t.Fatalf("insert early person: %v", err)
	}
	if _, err := s.write.ExecContext(ctx,
		"INSERT INTO devices (device_key, user_id, platform, created_at, last_seen_at) VALUES ('dev-early', 'u_early', 'test', 1, 1)"); err != nil {
		t.Fatalf("insert early device: %v", err)
	}
	if _, err := s.write.ExecContext(ctx,
		"INSERT INTO users (user_id, label, created_at) VALUES ('u_late', 'Late', 2)"); err != nil {
		t.Fatalf("insert late person: %v", err)
	}
	// The owner is set only after that person exists: the foreign key refuses
	// to point at nobody, which is the constraint doing its job.
	if _, err := s.write.ExecContext(ctx,
		"UPDATE server_identity SET owner_user_id = 'u_late', claimed_at = 2 WHERE id = 1"); err != nil {
		t.Fatalf("set owner: %v", err)
	}

	early, err := s.ResolveIdentity(ctx, "dev-early", "", 300)
	if err != nil {
		t.Fatalf("ResolveIdentity: %v", err)
	}
	if early.Owner {
		t.Fatal("the earliest person reports ownership they were never given")
	}
}
