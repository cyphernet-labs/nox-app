...
⚠️ **Стрелок, которых нет:** приложение ↔ relay-сервер (адреса релея в приложении не существует), байты файлов по сокету, push через сокет.
...
Значит, чтобы бэкенд пользователя мог послать пуш в NOX, ему нужны были бы **наши приватные ключи** — то есть их пришлось бы раздать всем пользователям, и любой, кто их достанет, сможет слать пуши всем клиентам NOX. Поэтому ключи лежат **в одном месте**, на relay-сервере, и пуш идёт только оттуда.

К тому же сокета в этот момент не существует: приложение убито, будить его умеет только ОС. К этому же независимо пришли Zulip, Mattermost, Rocket.Chat, Prosody и Delta Chat — разбор в `docs/research/snikket-fit.md` §4.

**Пользователю это не стоит ни одного шага:** в мастере установки бэкенда push не упоминается вообще.
...
Протокол «бэкенд ↔ релей» (другой инженер) · конверт и коды ошибок (после Q1) · push (`docs/research/snikket-fit.md` §4) · файловый HTTP-канал · экономика и юридический статус оператора.
