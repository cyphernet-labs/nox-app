# Подключение приложения к клиентскому бэкенду — Preview для команды

**Статус:** 🟡 Needs input · **Границы:** спаривание, формат ссылки, транспорт, установка бэкенда · **Риск:** средний · **Ключевое решение:** приложение соединяется напрямую со своим бэкендом и о центральном релее не знает

> **Что это.** Верхнеуровневое описание решения: из каких частей состоит, как они разговаривают, что между ними ходит, что решено и что открыто. Реализации пока нет — это база для обсуждения и последующей спецификации, а не план изменений в коде.
