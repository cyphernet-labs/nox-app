# Speech AI Mobile — Архитектурный блюпринт

> **Назначение:** канонический референс архитектуры, паттернов и инфраструктуры мобильного приложения **Speech AI** (iOS + Android, Flutter), которое предстоит реализовать в `sources/mobile_app/` (сейчас — пустой плейсхолдер). Этот набор документов — единственный источник правды для любого разработчика (и для Claude Code) при имплементации **каждой** фичи приложения. **Когда читать:** перед началом работы над приложением целиком, перед любой новой фичей, и как справочник по конкретному слою/паттерну. **Связанные документы:** все 18 спутниковых файлов `00-architecture-overview.md` … `17-analytics.md` (таблица ниже).
>
> **Статус: рабочий блюпринт** — принят владельцем как авторитетный референс для `sources/mobile_app/` (2026-06-08). Любая mobile-задача проектируется и кодится **по этому набору**; найденные баги/неверные/расходящиеся реализации приводят блюпринт в корректный вид в том же change-set'е (правило `.claude/memory/feedback_mobile_blueprint.md`). Известные отложенные гэпы (фиче-слои MVP — плеер/Reading-tab, IAP, auth/app-state-spine, file-download, records/user/tracks-контракты, client Sentry, native deep-link, локализация) **по решению владельца сейчас не покрываются**; при заходе в mobile-фичу — пересобрать gap-анализ.

---

## Что это и что заменяет

Это **унифицированный** блюпринт, синтезированный из трёх ранних подходов и **полностью их заменяющий**:

- `prompts/migration/` — BASE (авторитетный, выигрывает по умолчанию);
- `prompts/migration_v1/` — более богатый вариант (из него взяты лучшие шаблоны);
- `prompts/migration_v2/` — референс по Freezed-for-BLoC и пагинации.

Все три каталога-источника **устарели** с появлением этого блюпринта. Если в них и здесь написано разное — авторитетен **этот** набор документов. Источники сохранены лишь как историческая справка; новый код пишется только по `docs/mobile/`.

Блюпринт **доменно-нейтрален**: он описывает архитектуру, слои, кодоген, тулинг и инфраструктуру, не привязываясь к конкретной бизнес-логике. Сквозной worked-пример — нейтральная фича `Item`. **Первая реальная фича**, которую предстоит собрать по этому блюпринту, — список записей (`records`-list клиентского backend'а, offset-пагинация); об этом упоминается там, где это уместно, но шаблоны остаются на нейтральном `Item`.

---

## Несущие инварианты (проекция конституции и золотых правил)

Это несущие инварианты. Всё остальное — стиль. Авторитетный свод — **9 принципов + 10 золотых правил** (см. [`08-conventions-and-constitution.md`](08-conventions-and-constitution.md)); список ниже — их операционная проекция в этом индексе. Два сознательных переопределения правил источников — **BLoC = Freezed** и **пагинация = `infinite_scroll_pagination` v5** — свёрнуты в существующие принципы/правила (не как новые отдельные пункты).

1. **ОДИН Dart-пакет, слои — это папки.** Никаких трёх пакетов и path-deps. Слои живут как директории внутри одного `lib/`: `lib/data`, `lib/domain`, `lib/presentation`, плюс `lib/di`, `lib/general`, `lib/design`, `lib/resource`. ОДИН `pubspec.yaml`, ОДИН прогон `build_runner`. Однонаправленные зависимости: `presentation → domain`, `data → domain`; **`domain` не импортирует ничего**. Любые трёхпакетные пути из источников (`domain/lib/src/...`, `data/lib/src/...`) переписываются в однопакетные (`lib/domain/...`, `lib/data/...`). См. `00-architecture-overview.md`.
...
3. **BLoC = Freezed.** `@freezed` `sealed`-юнионы для State и Event; тонкий `BaseBloc<E, S>` с `executeLogic` (try/catch). Производная/вычисляемая логика — в **extension-геттерах** (не в теле `@freezed`-класса). `copyWith` для переходов; `sealed` для много-вариантных юнионов, `abstract` для одно-вариантных value-объектов; **никакого `fromJson` на BLoC-типах** (только `*.freezed.dart`, никогда `*.g.dart`). Это **сознательно переопределяет** рукописный `sealed` + `Equatable` BLoC из BASE и v1 и старое правило «без Freezed для state». Канонические имена под-состояний — `Initializing` / `Initialized` / `Error` — сохраняются и выражаются Freezed-`const factory`-конструкторами. См. `05-presentation-layer.md`.

4. **Пагинация = `infinite_scroll_pagination` ^5.1.1** (v5, stateless `PagingState`-в-bloc, **никогда** `PagingController`). Переиспользуемое расширение `PagingStateExt.applyPage`. **Дефолтный flavor — OFFSET** (`PageMetadata{int? nextPage, int total}`), потому что records-list клиентского backend'а — offset-based (`page` + `page_size` + `count`); CURSOR (`CursorPaginationMetadata{String? nextCursor}`) документируется как альтернатива. Репозиторий возвращает `RepositoryResult<(List<T>, PageMetadata)>`; `result.exception` прокидывается в `pagingState.error` для v5-error-builder'ов. См. `07-pagination.md`.
...
6. **Трёхчастный data-split.** API JSON ↔ `Entity` (Freezed **+** `json_serializable`, basic-types-only) ↔ `Mapper` ↔ доменная `Model` (**только** Freezed, без JSON). Бизнес-логика — в `extension`'ах на модели, не в теле Freezed-класса. Сохраняется `ResponseEntity<T>` + рукописный реестр `EntityConverter<E>` (реальный backend отдаёт унифицированный конверт `{data, timestamp, trace_id, meta}`). Вся коэрция типов (`enum` как `.name` String, `DateTime` как ISO-8601 String) — в маппере. См. `04-data-layer.md`.
...
8. **Только design-токены.** Никаких хардкод-`Color`, `EdgeInsets`, `TextStyle` или system-overlay-style в коде фич. Тема — **light + dark** через `ThemeExtension<AppColors>` + `AppTheme.light()/dark()` + `context.appColors` + `themeMode` из `AppRootBloc` (механизм v1), но с конкретной палитрой и `flutter_screenutil`-токенами spacing/typography + `AppOverlayStyleTokens` из BASE. Гибрид. См. `06-theming.md`.
...
|---|---|---|
| Имя Dart-пакета | `pubspec.yaml` `name:`; все импорты `package:speech_ai_mobile/...` | `speech_ai_mobile` *(владелец может переименовать — отмечено только здесь, в README)* |
| Отображаемое имя | App display name | `Speech AI` |
| `applicationId` / iOS bundle (prod) | Android `applicationId`, iOS bundle id | `app.speechai.mobile` |
| `applicationId` / iOS bundle (stage) | то же для stage-flavor | `app.speechai.mobile.stage` |
...
| 14 | [`14-networking-and-auth.md`](14-networking-and-auth.md) | Сеть и авторизация: `ApiClient` (Dio + auth-interceptor), `AppConfigRepository` (флейвор-конфиг + источник токена), мост к REST-слою; §4 — адаптация под `client_backend` (HMAC-подпись + security-заголовки + access/refresh токен-модель); §6 — connectivity + app-lifecycle. |
| 15 | [`15-push-notifications.md`](15-push-notifications.md) | Push-уведомления (FCM): `firebase_messaging`, device-токен и его ротация, регистрация/разрегистрация на backend, foreground/background/terminated, разрешения, навигация по тапу (через 13). |
| 16 | [`16-file-upload.md`](16-file-upload.md) | Загрузка контента: 3-step (upload → estimate → create record), file/image picker, квота/`can_process`, идемпотентность `sources_signature`, BLoC-прогресс. |
| 17 | [`17-analytics.md`](17-analytics.md) | Клиентская аналитика (Mixpanel): `AnalyticsRepository` (интерфейс + impl), `@freezed`-таксоном `AnalyticsEvent`, super-properties, приватность (opt-out, без PII), точки трекинга. |
...
17. `16-file-upload.md` — загрузка контента (3-step upload → estimate → create).
18. `17-analytics.md` — клиентская аналитика (Mixpanel).
...
- **ОДИН Dart-пакет** `speech_ai_mobile`; слои — папки внутри `lib/`; один `pubspec.yaml`; один `build_runner`; один DI-init.
...
- **Тема light + dark** через `ThemeExtension<AppColors>` (`AppTheme.light()/dark()`, `context.appColors`, `themeMode` из `AppRootBloc`) с палитрой и `flutter_screenutil`-токенами из BASE.
- **Пагинация = `infinite_scroll_pagination` v5** + `PagingStateExt.applyPage`, дефолтный flavor — **OFFSET** (`page`/`page_size`/`count` records-list), CURSOR — альтернатива.
...
- [ ] Понимаете назначение: канонический блюпринт архитектуры iOS+Android Flutter-приложения для `sources/mobile_app/`, заменяющий `prompts/migration*`.
- [ ] Знаете авторитетный свод — **9 принципов + 10 золотых правил** (см. [`08-conventions-and-constitution.md`](08-conventions-and-constitution.md)) — и что **BLoC = Freezed** и **пагинация = `infinite_scroll_pagination` v5** сознательно переопределяют старые подходы (свёрнуты в существующие правила).
- [ ] Усвоили соглашения имён: пакет `speech_ai_mobile` (заменяем), worked-пример `Item`, пустые скелеты `<Feature>`/`<Model>`, `app.speechai.mobile` (+`.stage`), flavor'ы `stage`/`prod`.
