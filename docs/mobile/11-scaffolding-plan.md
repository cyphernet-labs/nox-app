...
- **Первая РЕАЛЬНАЯ фича**, которую будете строить после скелета, — список записей (`records` из client_backend, offset-пагинация: `page` + `page_size` + `count`). Скелет на `Item` спроектирован так, чтобы `records` лёг в него один-в-один (см. §10 и `07-pagination.md`).

### Соглашения по именам (зафиксированы для всего блюпринта)

| Сущность | Значение |
|---|---|
| Имя Dart-пакета | `speech_ai_mobile` (все импорты — `package:speech_ai_mobile/...`) |
| Отображаемое имя | Speech AI |
| Android `applicationId` / iOS bundle | `app.speechai.mobile` (stage: `app.speechai.mobile.stage`) |
| Флейворы | `stage`, `prod` |
| Flutter / Dart | Flutter 3.44.1 через FVM; Dart sdk `>=3.12.0 <4.0.0` |
| Длина строки форматтера | 140 |

### Ключевое архитектурное отличие от исходников

Этот скелет — **ОДИН Dart-пакет** (`speech_ai_mobile`). Слои — это **папки** в одном `lib/`: `lib/data`, `lib/domain`, `lib/presentation`, плюс `lib/di`, `lib/general`, `lib/design`, `lib/resource`. **ОДИН** `pubspec.yaml`, **ОДИН** прогон `build_runner`, **ОДИН** `configureDependencies(String env)` с одним `@InjectableInit(initializerName: r'$initGetIt')` и одним сгенерированным `configure_dependencies.config.dart`. Никаких трёх пакетов, path-зависимостей, цикла `domain↔data`. Если в исходных шаблонах встречаются пути вида `domain/lib/src/...` или `data/lib/src/...` — они переписываются на `lib/domain/...` / `lib/data/...`.
...
**Действие.** Создать корень проекта `sources/mobile_app/`, закрепить точную версию SDK, чтобы каждый разработчик и CI-раннер использовали один Flutter. Все скрипты скелета вызывают `fvm flutter` / `fvm dart` — никогда захардкоженный путь к SDK.
...
- `pubspec.yaml` (единственный) — `name: speech_ai_mobile`, `environment: sdk: '>=3.12.0 <4.0.0'`, объединённые зависимости всех трёх слоёв.

**Команды.**

```bash
# Создаём РЕАЛЬНЫЙ Flutter-проект (нужны платформенные runner'ы для flutter build/test).
fvm flutter create --org app.speechai --project-name speech_ai_mobile .
...
**Не тянем (специфика исходников):** `ffi`, `flutter_rust_bridge`, `desktop_multi_window`, `yaru`, `flex_color_picker`, `decimal`, `custom_adaptive_scaffold`, любые crypto/wallet/RGB/FFI/multi-window зависимости.
...
**Действие.** Собрать переиспользуемый фундамент `lib/data/`: унифицированный конверт ответа бэкенда, реестр конвертеров entity, базовый маппер, реактивный Sembast DAO + env-scoped `AppDatabase`, Dio-клиент, типизированные исключения и `BaseRepositoryHelper` с **обязательным** `LogRepository`.

**Файлы** (см. `10-code-templates.md` → «Data base»; авторитетные шаблоны самого data-слоя — в `04-data-layer.md`):
- `lib/data/entity/base/response_entity.dart` — `ResponseEntity<T>`, разбор реального конверта `{data, timestamp, trace_id, meta}`.
...
- `lib/domain/model/app_config/app_config_model.dart` (+ impl) — держит конфиг флейвора (apiUrl, apiSignatureKey, appwriteProjectId и пр.; см. `02-dependency-injection.md` §7.3).
...
- `lib/design/theme/app_colors.dart` — `AppColors extends ThemeExtension<AppColors>` с `LightAppColors` / `DarkAppColors` + extension `context.appColors`. Палитра — конкретные значения из базы.
...
- `lib/resource/app_theme.dart` — `AppTheme.light()` / `AppTheme.dark()`, композирует Material-базу + `extensions: [LightAppColors()/DarkAppColors()]`. `themeMode` поставляется `AppRootBloc` (механизм v1).
...
- `lib/general/text_constants.dart` — `TextConstants` (имя приложения «Speech AI» + общие строки UI).
...
- `android/app/build.gradle.kts` — flavor dimension, детект флейвора, выбор keystore, pre-build decrypt-хук; `applicationId` `app.speechai.mobile` (suffix `.stage`).
- iOS — schemes (`stage`/`prod`) + xcconfig-подстановка + bundle id `app.speechai.mobile[.stage]`.
...
- [ ] Сборка обоих флейворов (`stage`/`prod`) проходит; `applicationId`/bundle = `app.speechai.mobile[.stage]`.
...
- `CLAUDE.md` (скелет проекта `sources/mobile_app/`) — золотые правила, нейминг, HARD RULE форматирования на 140, single-package/Freezed-BLoC/pagination инварианты, заметка о ведении `entity_converter.dart`.
...
- [ ] Один пакет `speech_ai_mobile`: слои — папки в одном `lib/`; один `pubspec.yaml`; `pub get` чистый; нет path-зависимостей/цикла.
...
- [ ] Секреты SOPS+age+mise; флейворные сборки `stage`/`prod` (`app.speechai.mobile[.stage]`); версионирование CalVer + shifted-epoch.
