...
На десктопах push **существует** (APNs на macOS, WNS на Windows), но решением владельца не используется — §7.
...
**Push на десктопах есть, и мы его сознательно не берём.** Раньше здесь стояло «push-пути нет вовсе» — это было неверно для macOS и Windows.
...
**Решение владельца (2026-08-16): push на десктопах не используем** — цена несоразмерна единственному сценарию, который он закрывает.

Тот же выбор сделали все восемь разобранных десктопных клиентов (Signal, Element, Telegram, WhatsApp, Wire, Threema, Delta Chat, SimpleX): **ни один не использует вендорский push на десктопе**, все держат собственное постоянное соединение. Причём именно отказались, а не «не могли»: у Telegram в API определены типы токенов WNS и Web push.
