# Личность и устройства — шесть кейсов жизненного цикла
...
**Один артефакт.** Во всех кейсах, где что-то предъявляется, используется одна и та же ссылка (она же QR): `https://nox.app/p/#<payload>`, где payload = адрес + порт + публичный ключ сервера (32 байта) + одноразовый токен (16 байт, TTL). Формат — [client-backend-connect-solution.md](../preview/client-backend-connect/client-backend-connect-solution.md) §4. Один парсер, один сканер, один набор тестов; кейсы различаются только **типом токена** внутри.

| Тип токена | Кейс | Кто выпускает | Что даёт |
|---|:---:|---|---|
| `claim` | 1 | установка сервера | Владение сервером |
| `invite-user` | 2 | владелец сервера | Новая личность на этом сервере |
...
| 2 | Новый пользователь на существующем сервере | `invite-user` | 🔴 заблокирован Q4 |
| 3 | Новое устройство того же пользователя | `invite-device` | 🟡 механизм ясен |
| 4 | Восстановление: живых устройств нет | `recovery` | 🟡 машина — да; код восстановления — открыт |
...
sequenceDiagram
  ...
  M->>CB: первый запуск
  CB->>CB: пара ключей + самоподписанный сертификат + claim-токен
  CB-->>M: ссылка/QR (адрес, ключ сервера, claim)
  D->>D: скан QR, разбор payload
  D->>CB: wss + сверка ключа с QR
  D->>D: генерация пары ключей устройства
  D->>CB: pair {claim, публичный ключ устройства}
...
## 4. Кейс 2 — новый пользователь на существующем сервере

Существует только в модели «сервер на круг/семью» — **вопрос Q4 открыт**. Владелец добавляет человека в круг: тот получает собственную личность со своим списком устройств.
...
sequenceDiagram
  ...
  CB-->>O: ссылка/QR (адрес, ключ сервера, invite-user)
  O->>N: показать QR
  N->>CB: wss + сверка ключа, pair {invite-user, ключ устройства}
...
sequenceDiagram
  ...
  CB-->>P: ссылка/QR
  P->>N: показать QR
  N->>CB: wss + сверка ключа, pair {invite-device, ключ устройства}
...
sequenceDiagram
  ...
  CB-->>M: ссылка/QR (recovery-токен)
  N->>CB: wss + сверка ключа, pair {recovery, ключ устройства}
  CB->>CB: токен сжечь, ключ добавить к личности
  CB-->>N: доступ восстановлен, история — по границам хранения
```

🟡 Второй путь — **код восстановления**, выданный при первой установке (аварийный комплект): авторизует новое устройство без доступа к машине. Делаем ли — открытый вопрос Q16.
...
| | Вопрос | Реестр |
|---|---|---|
| 🔴 | Сервер на пользователя или на круг — существует ли кейс 2 вообще | Q4 |
| 🟡 | Политика приглашений в круге: только владелец / любой участник / со-владельцы (bus factor) | Q15 |
| 🟡 | Код восстановления и аварийный комплект (ключ сервера + recovery-код + напоминание о бэкапе) | Q16 |
