...
**Решение Q15 (владелец, 2026-09-04): приглашает только владелец — тот, кто забрал машину claim-ссылкой, — и приглашение срабатывает только после подтверждения на его устройстве.** Подтверждение принято именно здесь, а не для приглашения устройства: своё второе устройство человек держит в руках, а приглашение чужому уезжает через мессенджер, где живёт дольше и видно не только адресату.

Из решения следует три вещи, которых сегодня в сервере нет:

1. **Владельца надо назвать явно.** `users` не хранит ролей, и «первый» узнаётся только по тому, что его завёл claim-токен, — правилу «приглашает только владелец» не на что опереться.
2. **`pair` перестаёт быть мгновенным.** Предъявивший ждёт, устройства владельца получают событие, владелец подтверждает или отклоняет. Это единственное место в протоколе, где ответ зависит от действия человека, и таймауты здесь придётся выбирать под человека, а не под сеть.
3. **Отказ должен быть отличим.** «Ты не владелец» и «токен недействителен» — разные вещи для того, кто их читает.

Отзыв **человека** (не устройства) решением не покрыт: Q15 отвечает, кто впускает, и не отвечает, кто выпроваживает.
