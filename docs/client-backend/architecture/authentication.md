...
| `flags` | тип хоста (IPv4 / IPv6 / имя) + **тип токена** (см. таблицу ниже) | 1 байт |
| `port` | порт | 2 байта |
| `host` | адрес | 4 / 16 / переменной длины |
| `server_key` | публичный ключ client-сервера (Ed25519) | 32 байта |
| `token` | одноразовый токен, TTL. Обобщение `claim_token` из первичного дизайна: то же поле, тип задаётся в `flags` | 16 байт |
