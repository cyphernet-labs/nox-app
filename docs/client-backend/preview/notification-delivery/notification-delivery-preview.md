...
На мобильных push — единственный работающий триггер. На десктопах нативный push не используется: он технически сложен, завязан на сторонние сервисы и закрывает только один сценарий, а фоновый процесс с постоянным сокетом даёт то же самое проще.
...
## 4. ⚠️ Silent push — не тот механизм

Схема «silent push → идём на бэкенд → показываем локальное уведомление» **на Android работает буквально**. **На iOS — нет**, и это самая частая ошибка в этой области.
...
**Тот же UX достигается другим механизмом.** Релей шлёт alert-push с заглушкой в теле; iOS **до показа чего-либо** запускает расширение; расширение идёт в свой client-сервер и **переписывает** текст уведомления. Пользователь не отличит это от локального уведомления.

Три особенности iOS, которые надо принять:
