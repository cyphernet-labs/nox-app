...
│   ├── widgets/               ← per-widget Dart (primitives + composites)
│   │   ├── nox_primitives.dart
│   │   ├── nox_widgets.dart
│   │   ├── nox_scaffold.dart
│   │   └── preview.html       ← live gallery: real components + their Dart
│   └── README.md
└── spec/                      ← component & screen specs (widget mapping + token bindings)
    ├── components.md
    ├── primitives.md          ← per-widget description + API + token bindings
    ├── screens.md
    └── icons.md
```
...
**C — Build the widgets.**
`flutter/widgets/*.dart` implements every NOX primitive as an idiomatic M3 widget on top of the
theme. `spec/primitives.md` describes each (API + token bindings); `flutter/widgets/preview.html`
renders the real components next to their Dart. Stock-themed widgets (buttons, text field, dialog)
aren't reimplemented — the theme styles them.
