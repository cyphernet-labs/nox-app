...
> **Settings** · desktop (macOS / Windows / Linux) · Material 3 · new in feature 032
...
## Anatomy
Fills the Settings detail pane (7.1); no push, selection swaps the pane. Current device in its own group, marked `This device`. Other devices in a second group, or the line `No other devices`. A filled `Add a device` button at the bottom; pressing it mints an invite and shows a card with the QR **and** the link as selectable text.
