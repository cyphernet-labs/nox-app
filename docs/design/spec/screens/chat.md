...
> Добавлено при реализации M4. Сведено с `nox-desktop-screens/screens/01-chats.md`.

- Десктоп (`>= 840dp`): лента — **правый thread-pane** внутри list-detail списка чатов (5.1), не отдельный full-screen. Persistent **ThreadHeader** (`AppThreadHeaderWidget`): аватар + имя чата (тап → 5.4) + info-действие (`folderOpen` → 5.4). Поток сообщений — в колонке чтения ≤980; composer снизу. Выбор чата в 5.1 загружает ленту **без push** (заменил M3-плейсхолдер).
- **Реконсиляция под NOX-модель:** ThreadHeader НЕ показывает участников / per-chat-поиск / папки (упоминания в десктоп-корпусе — дрейф: открытое пространство без участников/подписок, per-chat-поиск/папки вне scope). Этот спек — источник истины; реализация приведена к нему (Принцип II).
