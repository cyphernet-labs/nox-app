...
> ⚠️ **С фазы 032 сканер читает ссылку спаривания**, а не `nox://id/`. Полезная нагрузка передаётся дальше **целиком**: из неё ничего не извлекается, потому что адрес сервера и его ключ нужны так же, как токен. На Windows и Linux камеры нет — там тот же разбор работает поверх выбранного файла, и это единственный путь; вставка текстом обязана работать везде.
