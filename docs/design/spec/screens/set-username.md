...
Имя — лимит 32 символа, charset — **латиница + цифры + `-`, `_`, `.`** (см. [overview.md](../overview.md#идентичность-пользователя)). **Уникальным оно не является.**

> ⚠️ **Уникальности имён нет и не будет** (решение владельца 2026-09-02). Требование «глобально уникальное имя» отменено, а не отложено: сервер не проверяет ни длину, ни набор символов, ни занятость label, и **не может отказать в приветствии из-за имени** (контракт §3) — отказ в приветствии клиент повторяет бесконечно, и проверка без переделки этого поведения дала бы вечный цикл вместо внятной ошибки. Экран поэтому не имеет состояния «имя занято»: проверять нечему. Человека узнают по спаренному ключу устройства (фаза 032), а не по имени, — на чём требование и держалось. Фаза 031 сняла локальную проверку, которая сравнивала имя с четырьмя захардкоженными строками, то есть отказывала по правилу, которого никто не соблюдал.
...
| Q4 | Регистр | Регистр сохраняется как введён; сравнивать имена не с чем — они не уникальны |
| Q5 | Резервированные имена | Не запрещаем ничего; только charset и длина |
