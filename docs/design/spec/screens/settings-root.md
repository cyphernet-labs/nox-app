...
2. **Body:** прокручиваемый `ListView`:
   - **Identity card (Material Card):** визуально выделена.
     - ⚠️ **Отдельного «блока имени» с подписью `Name` и pencil-глифом больше нет** — имя стоит в шапке карты (см. ниже), а редактирование вызывается названной кнопкой `Edit name`. ⚠️ **С фазы 037 отметки владения (`Server owner`) тоже нет**: на сервере живёт один человек, и признак, у которого один возможный ответ, не различает ничего — он только выглядит информацией.
   ...
   - **Назначения — по одной плитке на пункт** (скруглённый `surfaceContainerLow`, радиус `lg`, зазор 8), без group-заголовков и без hairline'ов. У каждой — **ведущий глиф** (22, `onSurfaceVariant`) и **шеврон справа**:
     - `Devices` → 7.8;
     - `Notifications` → 7.2;
     - `Appearance` → 7.3;
     - `Language` → 7.4;
     - `Terms` → 7.6;
     - `About` → 7.7;
   - **`Log out` — своя плитка** с отбивкой, целиком в `ColorScheme.error`: глиф — **всегда filled**, шеврона нет (строка открывает диалог, а не ведёт куда-то).

     ⚠️ Иконки вернулись **по решению владельца от 2026-09-20**, отменившему «строки настроек без иконок» из визуального аудита 2026-08. Источник — дизайн-проект Claude Design (`NOX - Desktop.html`).

     ⚠️ **Круглой подложки под глифом нет** — тоже решение владельца, принятое по факту: корпуса рисуют глиф в тонированном круге 40dp, но шесть колец подряд читаются как шесть кнопок, а в десктопной панели кольцо конкурировало с пилюлей выделения вокруг него.

     ⚠️ История двух неверных попыток, чтобы к ним не вернулись. Сначала строки лежали **голыми на фоне скаффолда** — единственное на экране, что не было в контейнере, под карточкой аккаунта, которая была: читалось как недоделанный список. Потом их **слили в одну карточку с hairline'ами** — и получилось три разделителя, делающих одну работу: край карточки, линия и пилюля выделения; на десктопе пилюля вдобавок **вылезала через скруглённый угол карточки** (стадион с отступом 8 шире у торцов, чем угол, в котором он сидит). Теперь каждый пункт — самостоятельный объект.

### Десктопная меню-панель — `NavigationDrawer` (M3)

Пункты — стадионы **прямо на панели**, прозрачные, пока не выбраны; выбранный заливается `secondaryContainer`, его глиф перекрашивается в `onSecondaryContainer` и меняется на **filled** (та же ось FILL, что и у вкладок нижней панели). Карточки нет: панель уже отделяет список от detail-панели рядом, а карточка вокруг добавляла второй край, через который пробивалась пилюля.

Три группы, как в десктопном корпусе — `Account`+`Devices` · `Notifications`+`Appearance`+`Language` · `Terms`+`About` — разделённые **линией под группой**, а не между соседними строками: линия лежит ниже внутреннего отступа группы и пилюли не пересекает. `Log out` прижат к низу через `Spacer`.
...
| ~~Name block label~~ | ~~`Name`~~ — подписи нет: имя стоит в шапке карты под аватаром |
| ~~Name edit tooltip~~ | ~~`Edit`~~ — глифа нет, есть кнопка `Edit name` |
| Identifier label | `Your ID` — **не виден на экране**: только accessible-имя строки идентификатора (`Semantics`), потому что рядом с именем человека и кнопкой `Copy ID` подпись ничего не добавляет глазу, а скринридеру без неё достаются сорок непрозрачных символов без названия |
| ~~Identifier mask~~ | ~~`••••••••`~~ — маски нет с фазы 032, ID показан целиком |
| ~~Copy tooltip~~ | ~~`Copy`~~ — глифа нет, есть кнопка `Copy ID` |
...
- **ID показан целиком на обеих ширинах.** Ни маски, ни `Show/Hide`, ни QR-действия: с фазы 032 это публичный author id, прятать нечего. Действий у карты два — `Edit name` и `Copy ID`.
