...
2. **Body:** прокручиваемый `ListView`:
   - **Identity card (Material Card):** визуально выделена.
     ...
     - **Карта-шапка аккаунта**: аватар с инициалами и кольцом (`avatarLg` 56, те же инициалы, что в шелле) → имя (`titleLarge`) → **полный публичный идентификатор личности** моноширинным (`onSurfaceVariant`) → две tonal-кнопки `Edit name` и `Copy ID` в `Wrap`. ⚠️ **С фазы 032 раскрытия нет**: строка перестала быть секретом — человек узнаётся по спаренному ключу устройства, — и маскировать её значило бы прятать то, что прятать нечего. **QR-действия в карте нет**: добавление устройства — отдельный экран **7.8 Устройства**, куда ведёт свой пункт списка.

     Раньше это были две строки «подпись над значением», каждая с голым глифом у дальнего края: идиома поля формы без формы, без контейнера, который читался бы как кнопка, а в десктопной панели — на ширину панели от значения, к которому глиф относился. Действия названы словами: карандаш надо угадывать, `Edit name` — нет.
   - **Карточка назначений** (`AppSettingsGroupWidget`, без group-заголовков) — `ListTile`-ы с hairline между ними, тот же контейнер, что и списки на 7.2 / 7.8:
     - `Devices` → 7.8;
     - `Notifications` → 7.2;
     - `Appearance` → 7.3;
     - `Language` → 7.4;
     - `Terms` → 7.6;
     - `About` → 7.7;
   - **`Log out` — в отдельной карточке**, в `ColorScheme.error`. Hairline внутри общего списка говорил «и ещё вот этот пункт», а необратимое действие — не «ещё один пункт».

     ⚠️ Раньше строки лежали **голыми на фоне скаффолда** — единственное на экране, что не было в контейнере, под карточкой аккаунта, которая была. Читалось как недоделанный список. **Обе ширины теперь собраны одинаково**: те же две карточки в десктопной меню-панели, где до этого было три подгруппы, разделённые hairline'ами во всю ширину панели — а линия, пересекающая выделенную пилюлю, выглядит сломанной.
...
- **Тап на `Copy ID`** → копирование идентификатора в буфер + snackbar `Copied to clipboard`.
- **Тап на `Edit name`** → inline-редактирование имени в карте; кнопка на это время убирается (состояние, в которое она предлагает войти, уже наступило).
...
- `FilledButton.tonal` — `Edit name`, `Copy ID` (действия карты идентичности).
