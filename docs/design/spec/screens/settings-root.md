...
2. **Body:** прокручиваемый `ListView`:
   - **Identity card (Material Card):** визуально выделена.
     - **Блок имени**: label `Name` + текущее имя + **отметка владения** + edit-аффорданс (pencil-иконка). Тап превращает текстовую строку в inline `TextField`. Отметка (`Server owner`) показывается, только когда сервер прямо сказал, что этот человек им владеет: «не сказал» и «не владелец» рисуются одинаково — то есть никак, — потому что нарисовать «не владелец» до ответа сервера значит соврать, а потом моргнуть.
   ...
   - **Плоский список** `ListTile`-ов (без group-заголовков):
     - `Devices` → 7.8;
     - `People` → 7.9 — **только у владельца сервера**. У остальных строки нет вовсе: отказ на проводе существует ради честности протокола, а не как способ сообщить человеку о запрете. «Сервер не сказал» скрывает её так же, как «не владелец»;
     - `Notifications` → 7.2;
     - `Appearance` → 7.3;
     - `Language` → 7.4;
     - `Terms` → 7.6;
