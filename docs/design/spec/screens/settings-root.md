...
2. **Body:** прокручиваемый `ListView`:
   ...
   - **Назначения — по одной плитке на пункт** (скруглённый `surfaceContainerLow`, радиус `lg`, зазор 8), без group-заголовков и без hairline'ов:
     - `Devices` → 7.8;
     - `Notifications` → 7.2;
     - `Appearance` → 7.3;
     - `Language` → 7.4;
     - `Terms` → 7.6;
     - `About` → 7.7;
   - **`Log out` — своя плитка** с отбивкой, в `ColorScheme.error`.
...
Пункты — стадионы **прямо на панели**, прозрачные, пока не выбраны; выбранный заливается `secondaryContainer`. Ни карточки, ни hairline'ов: панель уже отделяет список от detail-панели рядом, а карточка вокруг добавляла второй край, через который и пробивалась пилюля. `Log out` прижат к низу через `Spacer`.
