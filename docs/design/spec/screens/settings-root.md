...
2. **Body:** прокручиваемый `ListView`:
   ...
   - **Назначения — по одной плитке на пункт** (скруглённый `surfaceContainerLow`, радиус `lg`, зазор 8), без group-заголовков и без hairline'ов. У каждой — **ведущий чип 40dp** (`secondaryContainer`, глиф 22 `onSecondaryContainer`) и **шеврон справа**:
     - `Devices` → 7.8;
     - `Notifications` → 7.2;
     - `Appearance` → 7.3;
     - `Language` → 7.4;
     - `Terms` → 7.6;
     - `About` → 7.7;
   - **`Log out` — своя плитка** с отбивкой, целиком в `ColorScheme.error`: чип — `error` на 14%, глиф — **всегда filled**, шеврона нет (строка открывает диалог, а не ведёт куда-то).
...
Пункты — стадионы **прямо на панели**, прозрачные, пока не выбраны; выбранный заливается `secondaryContainer`, его чип — `onSecondaryContainer` на 12%, а глиф меняется на **filled** (та же ось FILL, что и у вкладок нижней панели). Карточки нет: панель уже отделяет список от detail-панели рядом, а карточка вокруг добавляла второй край, через который пробивалась пилюля.
