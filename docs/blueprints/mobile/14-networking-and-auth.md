...
>
> **Реальный код vs целевой паттерн.** §1–4 описывают **целевой** сетевой контракт. Реальный `ApiClient` намеренно тоньше: он живёт плоско в `lib/data/remote/api_client.dart` (не `lib/data/remote/api/base/...`), это `@lazySingleton` с конструктором `ApiClient(this._config)` (`AppConfigRepository`), двумя полями (`_config` + `final Dio dio`) и 30-сек таймаутами. `initBase()` у него — **идемпотентный метод экземпляра**, а не статическая фабрика: он ставит `dio.options.baseUrl` из `AppConfig.apiUrl` (когда тот непустой, **без** суффикса `/api/`) и один раз навешивает `AuthInterceptor` (§4-security-pipeline'а нет). Сейчас `ApiClient` **инертен для команд**: `apiUrl` уже непустой на stage-флейворе (`Environment.dev`) — там лежит адрес локального `noxd` из `config/stage.json` (`app.apiUrl`), в prod-флейворе значения нет, — но живой канал команд с фазы 026 это WS-конверт, поэтому `initBase()` из кода приложения не вызывается (только из тестов) и ни один data source не инжектит `ApiClient`: Dio зарезервирован под blob-обмен фазы 028. DI-флип 016 при этом **уже сделан** (фаза 026): `RealChatRemoteDataSource` / `RealMessageRemoteDataSource` забиндены на `[Environment.dev]`, моки сужены до `[prod, test]`. Doc-comment самого класса всё ещё несёт дореформенную нумерацию фаз («…a WS envelope arriving with feature 027 … the 016 DI flip (feature 028)») — в этой части он устарел. Структура `lib/data/remote/api/base/api_client.dart` + `BaseApiRepository` ниже — это **целевой** REST-каркас (один хост — у NOX один бэкенд). Все четыре сегодняшних API-класса — `GetItemsApi`, `GetChatsApi`, `GetMessagesApi`, `SendMessageApi` (моки, см. §3) — оба этих каркаса обходят.
...
> **Факты реализации.** `AppConfigRepositoryImpl` не подмешивает `BaseRepositoryHelper` (для конфигурации, не ходящей в сеть, он ничего не добавляет), хранит конфиг лениво (`AppConfig? _config`) и собирает его прямо в `initialize` как `_config = AppConfig(flavor: flavorType, apiUrl: AppFlavor.getApiUrl())`, а на чтение `config` до `initialize` бросает `StateError('AppConfigRepository.initialize was not called')`. Токен читается из `flutter_secure_storage` по ключу `auth_id_token` — **писателя пока нет** (его даст sign-in stage 2), поэтому фактически всегда `null` → запрос уходит без `Authorization`. `isTestEnvironment` приходит из DI (`@Named('isTestEnvironment')`, env-keyed в `RegisterModule`), а не захардкожен.
...
3. **UX-баннер — не транспортный гейт.** С фазы 026 баннер ведёт не connectivity напрямую, а `SessionPhaseService.watchPhase()` (`ChatsListBloc` / `ChatThreadBloc` / `ChatCardBloc`, см. §5.4): в окружении `dev` (флейвор `stage`) фазу отдаёт сам сокет (`SocketSessionPhaseService`), а под `[prod, test]` она остаётся проекцией `watchOnline()` (`ConnectivitySessionPhaseService`, `lib/data/service/session_phase_service_impl.dart`). Ни та, ни другая ветка на транспортный слой не влияет.
