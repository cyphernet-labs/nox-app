...
├── general/       → constants, text_constants, feature_flags, platform_utils, formatters
...
Логирование — единственным каналом `LogRepository` (`debug`/`error`). **Сырые `print`/`debugPrint` в `lib/` запрещены** (тесты — исключение). **Типизированной иерархии исключений нет:** есть маркер `abstract class BaseRepositoryException {}` + общий enum `RepositoryException implements BaseRepositoryException { unknown, internal, authentication, connection, unauthenticated, notFound }` (фиче-специфичные исключения, если появятся, реализуют тот же маркер отдельными enum). Необработанные ошибки транспорта/фреймворка **обязаны** маппиться в доменный `RepositoryException` через `BaseRepositoryHelper.execute<T>()` (`DioException → internal`, прочее → `unknown` — *транспорт NOX ещё не выбран; Dio здесь — размеченный пример*). `RepositoryResult.exception` всегда несёт подтип `BaseRepositoryException`, никогда «сырую» ошибку транспорта/фреймворка. `BaseRepositoryHelper.execute<T>()` **всегда** логирует через обязательный `LogRepository`. Намеренно проглоченное исключение обязано иметь inline-комментарий, почему это безопасно. См. [04-data-layer.md](04-data-layer.md).
...
> **Карвут:** пагинированные серверо-владеемые списки и одноразовые POST-ы — **network-only**: без DAO и без `BehaviorSubject`. Список чатов NOX (открытый общий шаринг-спейс; эндпоинт вида `GET /api/v1/chats/` — *пример, бэкенд/протокол NOX ещё не выбран; заменить на реальный контракт*) — именно такой случай: пагинированный серверный список → сетевой, без кэш-слоя (см. [07-pagination.md](07-pagination.md)).
...
В фиче-коде запрещены «сырые» `Color`, `EdgeInsets`, `TextStyle` и системный overlay-стиль. Используются только токены-классы (4 в коде): `AppSpacingTokens`, `AppTextStyleTokens`, `AppOverlayStyleTokens`, `AppImagesTokens` (все в `lib/design/`). **Цвет — НЕ токен-класс:** только `context.appColors` (`ThemeExtension<AppColors>`); публичного класса-палитры цветов (`AppColorsTokens`) **нет** (см. [06-theming.md](06-theming.md)). Картиночные ассеты канонически идут через `flutter_gen` (`lib/design/gen/assets.gen.dart` — `Assets.png/.svg`), а `AppImagesTokens` остаётся рукописным удобным реестром путей. Spacing/типографика — через `flutter_screenutil` (адаптивные токены). Темизация — **light + dark** через `ThemeExtension<AppColors>` + `AppTheme.light()`/`dark()` + `context.appColors` + `themeMode` из `AppRootBloc`. Новые интерактивные виджеты оборачивают интерактивный элемент в `Semantics(label:, button: true)`. Все статические тоглы — в **одном** `lib/general/feature_flags.dart` как `static const bool` (remote-config-флаги — вне области этого модуля). См. [06-theming.md](06-theming.md).
...
10. **Пагинация — стандарт v5.** `infinite_scroll_pagination ^5.1.1`, stateless `PagingState`-в-блоке (никогда `PagingController`), переиспользуемое расширение `PagingStateExt.applyPage`, 1-based page-индекс (`PageMetadata{required int total, int? nextPage}`, `nextPage = null` на последней странице); ошибки прокидываются в `pagingState.error`. (См. [07-pagination.md](07-pagination.md).)
...
| Токены дизайна | `App*Tokens` | `AppSpacingTokens`, `AppTextStyleTokens`, `AppOverlayStyleTokens`, `AppImagesTokens` | `lib/design/` |
...
abstract final class TextConstants {
  ...
  static const String errorGeneralMessage = "We couldn't complete your request. Please try again later.";
  static const String errorConnection = 'No internet connection.';
  static const String errorAccess = 'Access error. Please sign in again.';
  static const String errorNotFound = 'Not found.';

  /// <FeatureName>  — one section per feature
  // static const String itemListTitle = 'Items';
}
```
...
10. Пагинация — infinite_scroll_pagination ^5.1.1, stateless PagingState-в-блоке, PagingStateExt.applyPage,
    1-based (defaultPage=1); ошибки → pagingState.error.
