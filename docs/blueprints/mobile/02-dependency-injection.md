...
Ты **никогда** не пишешь `getIt.registerLazySingleton<T>(...)` руками для аннотированных классов — это делает генератор. Руками регистрируются только те немногие вещи, которые генератор вывести не может (см. §3 — например `PackageInfo`, который подключается с первым же потребителем; в текущем скелете ручных регистраций нет вовсе).
...
В **текущем скелете** файл минимален — единственный вызов генератора, никаких ручных регистраций:
...
> **Скелет vs целевой паттерн.** Сейчас тело — только `getIt.$initGetIt(environment: env)`; всё остальное аннотационно-управляемо (включая выбор `AppDatabase` по окружению, §5). Ручные регистрации, которые генератор вывести не может (например `PackageInfo`), подключаются **с первым же потребителем** — по правилу «no-op/disabled-стаб вводится с первым потребителем подсистемы, не раньше». Ниже — целевая форма этого блока, к которой код придёт, как только появится первый async-pre-resolve (или иной ручной) потребитель.

Целевая форма (прескриптивный паттерн, **ещё не в скелете** — добавляется при появлении первого ручного потребителя):
...
- `PackageInfo` (в целевой форме) — значимая ручная регистрация: на устройстве это async-резолв с `preResolve: true`, под тестом — синхронный мок до запуска чего-либо. В скелете его пока нет — резолв `getIt<PackageInfo>()` появится вместе с первым потребителем (а `getIt.allReady()` в `main.dart` до тех пор остаётся безопасным no-op, см. §10).
...
Интерфейс и реализации живут в `lib/data/local/` (подробно — в [04-data-layer.md](04-data-layer.md)); здесь — только DI-аспект. Фактический интерфейс в скелете — два метода:
...
> Скелет несёт ровно эти две операции (`db`-геттер + `clearEntireDatabase()`, удаляющий файл БД и сбрасывающий кэш-инстанс). Логаут-fan-out по всем DAO (`cleanData()` каждого DAO) — **целевая** механика, которая придёт с первой реально кэшируемой фичей; пока `cleanData()` есть только на `ItemDao` и `AppDatabase` его не оркестрирует. См. [04-data-layer.md](04-data-layer.md) §6.
...
> `ItemDao` — реактивная кэш-первичная (cache-first) ветка скелета (`watch()` по `onSnapshots`). В **network-only** `Item`-репозиторий (§6.2, carve-out для серверных пагинированных списков) он **пока не подключён** — DAO ждёт первой реально кэшируемой фичи. Полное тело — в [04-data-layer.md](04-data-layer.md) §6.
...
> Скелетный `Item`-репозиторий **network-only** (carve-out для серверных пагинированных списков, см. [04-data-layer.md](04-data-layer.md)): он инъектит `ItemMapper` + `GetItemsApi`, а **не** `ItemDao`, и экспонирует только `getItems(...)` + `clean()`. Полный cache-first CRUD (`fetchItem`/`watchItems`/`createItem` через `ItemDao` + `BehaviorSubject`) — это полная форма примера, которая приходит с первой кэшируемой фичей. Логирование — через миксин `BaseRepositoryHelper` (отдельного поля `LogRepository` в репозитории нет).
...
**Скелет (фактический код).** Сейчас флейвор-конфиг минимален: доменная модель `AppConfig` несёт **только флейвор**, а `AppConfigRepositoryImpl` строит её в `initialize(...)`. Никаких `const String.fromEnvironment`-геттеров пока нет — источник токенов / `apiUrl` / security-заголовки помечены примером/TBD (бэкенд/протокол NOX ещё не выбран).
...
> `AppConfigRepositoryImpl` зарегистрирован под всеми тремя окружениями (`env: [dev, prod, test]`) и **не имеет конструкторных зависимостей** — он просто хранит флейвор, заданный в `initialize(flavorType:)` из `main.dart` (§10). До вызова `initialize` геттер `config` бросает `StateError`. Под тестом он резолвится без ручных моков (отдельной test-конфиг-модели регистрировать не нужно).

**Целевая форма (пример/TBD — бэкенд/протокол NOX ещё не выбран).** Когда появится реальный бэкенд, тот же тип `AppConfig` обрастает дополнительными полями (`apiUrl`, источник токена, ключи подписи/безопасности и т. п.), а `AppConfigRepositoryImpl.initialize(...)` заполняет их из dart-define-payload'а (`const String.fromEnvironment`). **Per-flavor-подкласса нет** — все различия приходят из payload'а флейвора. Тип-носитель — **`AppConfig`** (плоский value-object), а не отдельная «конфиг-модель»; контракт чтения остаётся прежним (`AppConfigRepository.config`). Поля ниже — **пример/TBD** (заменить на реальный контракт NOX); сегодня `AppConfig` несёт только `flavor`.

```dart
import 'package:nox_app/domain/model/app_config/app_flavor_type.dart';

/// Flavor-dependent runtime config. TODAY this carries only `flavor` (см. скелет выше).
/// The fields below are example/TBD target fields, added once the NOX backend/protocol
/// is chosen — they are NOT shipped yet.
class AppConfig {
  const AppConfig({
    required this.flavor,
    // ── example/TBD target fields (NOX backend not chosen) ──
  ...
  // ── example/TBD target fields (NOX backend not chosen) ──
  // final String apiUrl;          // пример/TBD
  // final String apiSignatureKey; // пример/TBD (ключ подписи запросов)
  // final String backendProjectId;// пример/TBD
  // final String supportEmail;    // пример/TBD
}
```
...
class AppConfigRepositoryImpl implements AppConfigRepository {
  ...
  Future<void> initialize({required AppFlavorType flavorType}) async {
    _config = AppConfig(
      flavor: flavorType,
      // ── example/TBD: read each target field from the dart-define payload once the backend is chosen ──
      // apiUrl: const String.fromEnvironment('API_URL', defaultValue: ''),
...
>
> Реальный набор значений (`API_URL`, `API_SIGNATURE_KEY`, идентификатор проекта бэкенда и т. д. — **пример/TBD**, бэкенд/протокол NOX ещё не выбран; заменить на реальный контракт) для stage/prod описан в [09-build-and-secrets-infra.md](09-build-and-secrets-infra.md): источник истины — зашифрованные секреты (SOPS+age+mise), которые `dart-define-from-file` материализует в `.env.<flavor>.json` на этапе сборки.

---

## 8. Глобальные алиасы — `lib/di/global_aliases.dart`

Вручную поддерживаемые top-level **геттеры**, оборачивающие `getIt<T>()` для часто используемых синглтонов. Применяются внутри реализаций репозиториев и API-классов, чтобы не писать `getIt<...>()` каждый раз. В скелете их **ровно два** — алиасы добавляются по мере того, как синглтон становится «горячим» (часто резолвится), а не заранее:
...
- Проброс в observability-бэкенд (Sentry / RUM) — точка расширения внутри `error(...)`, помеченная примером/TBD (бэкенд NOX ещё не выбран).
...
4. **`getIt.allReady()`** — блокирует до завершения всех `preResolve: true` async-регистраций (например, `PackageInfo.fromPlatform()` из целевой формы §3). Без него `getIt<PackageInfo>()` мог бы бросить «not ready». В текущем скелете async-pre-resolve-потребителей ещё нет, поэтому вызов — безопасный no-op; он оставлен на будущее, чтобы первый такой потребитель не требовал правки `main.dart`.
...
- `AppConfigRepositoryImpl` зарегистрирован под `test` (`env: [dev, prod, test]`) и не имеет конструкторных зависимостей — резолвится без ручных моков (отдельной test-конфиг-модели регистрировать не нужно).
...
- [ ] Реализации репозиториев — `@LazySingleton(as: <Feature>Repository, env: [Environment.dev, Environment.prod, Environment.test])`; **env-список не забыт** (иначе резолв бросает в рантайме). Скелетный `ItemRepositoryImpl(this._itemMapper, this._getItemsApi)` — network-only (`getItems` + `clean`), без `ItemDao`.
- [ ] `AppFlavorType{prod, stage}` + `AppFlavor.getFlavor()` (compile-time, `String.fromEnvironment('app.flavor')`, `default → prod`).
- [ ] Скелет: `AppConfig({required flavor})` (только флейвор) + `AppConfigRepositoryImpl` `@LazySingleton(as: AppConfigRepository, env: [dev, prod, test])` без конструкторных зависимостей, строит `AppConfig` в `initialize(flavorType:)`. Доп. поля `AppConfig` (`apiUrl`, источник токена, ключи подписи) с чтением через `const String.fromEnvironment` внутри `initialize` — целевая форма (пример/TBD, бэкенд NOX не выбран); тип-носитель остаётся `AppConfig`, контракт `AppConfigRepository.config` неизменен.
...
- [ ] `global_aliases.dart` — top-level **геттеры** (не `final`); в скелете ровно два (`logRepository`, `itemRepository`), остальные добавляются по мере «разогрева»; BLoC'и зовут `getIt<T>()` напрямую.
