...
### 5.4 Семантика `cacheOnly` (tri-state) и `cachedOnly` (двузначный)

Поле `cacheOnly` несут конфиги одиночных кэшируемых ресурсов (`GetItemConfig`). У замороженного среза `Item` списочный `GetItemsConfig` его не несёт — это `page` + `search`. Продуктовые списочные конфиги с фазы 026 несут **двузначный** флаг `cachedOnly` (`@Default(false) bool`): `GetChatsConfig` — `page` + `search` + `cachedOnly`, `GetMessagesConfig` — `chatId` + `beforeSeq` + `limit` + `cachedOnly`. Он нужен реактивному тику: перечитать уже загруженное окно из кэша, не отправляя команду на каждое событие. Там, где присутствует трёхзначный `cacheOnly`, семантика такая:
...
- [ ] `lib/domain/model/app_config/{app_config,app_flavor,app_flavor_type}.dart` + `lib/domain/repository/app_config/app_config_repository.dart` — flavor-зависимый рантайм-конфиг (`AppConfig({ required AppFlavorType flavor, String? apiUrl })`, `enum AppFlavorType { prod, stage }`; `apiUrl` заполнен с фазы 026 (транспорт + DI-флип; флейвор `stage`, который поднимает окружение `dev`, несёт адрес локального `noxd` из `config/stage.json`, прод — `null`), источник auth-токена ждёт stage-2-аутентификацию; см. `02`/`09`).
