...
> **`register`/`unregister` возвращают `bool`, а ошибки ловит `execute`.** На non-2xx Dio бросает `DioException` → `RepositoryException.internal` (нет `ApiException`, [04-data-layer.md](04-data-layer.md) §5). Конкретный код (например `429` rate-limited → ретрай с backoff) при необходимости разбирается явным `return RepositoryResult.error(...)` внутри callback'а после проверки `response.statusCode` — как в [14-networking-and-auth.md](14-networking-and-auth.md) §4.4.
