...
В keychain кладётся то, что даёт **доступ**. Признак владения сервером (`session.is_owner`, фаза 033) доступа не даёт: он заявляет роль, а решения по ней принимает сервер, — поэтому живёт в `SharedPreferences` рядом с `session.onboarding_complete`, `session.label` и `session.author_id`. Объявить его секретом значило бы платить платформенным каналом на каждом чтении за защиту того, что защищать нечего.
