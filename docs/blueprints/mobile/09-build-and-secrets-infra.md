...
В keychain кладётся то, что даёт **доступ**. Остальное живёт в `SharedPreferences`: `session.onboarding_complete`, `session.label`, `session.author_id`. Объявить открытые данные секретом значило бы платить платформенным каналом на каждом чтении за защиту того, что защищать нечего.

Ключ `session.is_owner` из фазы 033 **удалён фазой 037**: машина принадлежит одному человеку, и признак владения перестал что-либо различать. Он не пишется и не читается, а при первом чтении сессии подметается — установка, обновлённая со старой сборки, иначе носила бы мёртвый ключ до конца жизни устройства.

Хранится он вообще ради одного: запуск **офлайн** должен показывать последнее услышанное состояние. Иначе владелец, открывший приложение без сети, увидел бы себя не-владельцем — а «не знаю» рисовать как «нет» нельзя.

Стирается вместе с сессией: и `clear()` (выход), и `discardSignIn()` (несостоявшийся вход) убирают ключ.
...
class AppConfigRepositoryImpl implements AppConfigRepository {
  ...
  // … getUserAuthIdToken / limits / updateLimits / isTestEnvironment
}
```
