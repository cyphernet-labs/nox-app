...
> Транспорт выбран: контракт v0 ходит по **WebSocket-конверту**, и сам конвертный клиент приехал с феатурой 026. А вот защита канала — целевая продовая форма `wss` на 443 с pinning'ом ключа — **ещё не построена**: `socket_channel_factory.dart` открывает голый `IOWebSocketChannel.connect(url, pingInterval: 25s)` без `SecurityContext`, без `badCertificateCallback` и без сверки отпечатка, а stage смотрит на локальный `http://127.0.0.1:8080` (то есть `ws`, не `wss:443`). REST остаётся **только** для блобов — `file.uploadBegin` → `PUT` → `message.send{attachment:{file_id}}` и `file.downloadBegin` → `GET` с `Range` (одноразовые 10-минутные токены). Поэтому Dio-обвязка ниже — это путь загрузки/скачивания файлов, а не основной канал команд. Схема авторизации в интерсепторе (`Authorization: Bearer <token>`, `getUserAuthIdToken`) остаётся **примером**: аутентификация/пейринг — это stage 2 контракта, он ещё не закрыт. У NOX **один** бэкенд-хост, поэтому `ApiClient` несёт **единственный** Dio/host (`initBase` / `baseClient`); второго хоста нет.
