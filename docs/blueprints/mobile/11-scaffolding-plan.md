...
- `lib/data/remote/api_client.dart` — тонкая Dio-обёртка `ApiClient`: `initBase()` идемпотентно ставит base URL из `AppConfig.apiUrl` и один раз вешает `AuthInterceptor`. Инертна, пока `apiUrl == null`; реальный endpoint приходит с транспортом (фаза 027), а инъекция клиента в data-source'ы — с DI-флипом 016 (фаза 028).
- `lib/data/remote/request_builder/base/request_builder.dart` + `request_builder_helper.dart` — базовый builder запросов. **TARGET, а не шаг скелета:** этой папки в `lib/` нет и она не появилась вместе с бэкендом — контракт v0 идёт WS-конвертом (фаза 027), а REST сузился до blob upload/download. Создавайте её только если REST-поверхность вырастет; шаблоны — `04-data-layer.md` §7в.
...
- `lib/domain/model/app_config/app_config.dart` — `AppConfig({required AppFlavorType flavor, String? apiUrl})` (плоский класс: флейвор + nullable base URL; `apiUrl == null` во всех флейворах — реальный endpoint контракта v0 приходит с транспортом, фаза 027). Источник токена живёт не здесь, а в `AppConfigRepository.getUserAuthIdToken()`; сам механизм аутентификации/пэйринга — stage 2 и всё ещё открыт. Контракт — `lib/domain/repository/app_config/app_config_repository.dart` (интерфейс), реализация — `lib/data/repository/app_config/app_config_repository_impl.dart` (`@LazySingleton(as: AppConfigRepository, env:[dev,prod,test])`, без ctor-инъекций, метод `initialize(...)`). (`AppConfigModel` как тип **не существует** — это TBD-цель.)
