...
| `dio` | HTTP-клиент **REST-поверхности** client-сервера (`noxd`, Go + встроенный SQLite): `PUT /files/{upload_token}` и `GET /files/{download_token}` c Range — контракт v0 §1/§7. Основной канал команд и событий — не HTTP, а WebSocket-конверт: его клиент приехал в фазе **026** (`lib/data/remote/socket/`), поэтому dio остаётся только на файловой цепочке (фаза 028). **С фазы 036 dio ходит по тому же проверенному каналу, что и сокет**: `IOHttpClientAdapter` получает единственный `HttpClient` из `PinnedHttpClient`, у которого пустое хранилище доверенных корней и сверка отпечатка в `badCertificateCallback`. Порт не 443, а тот, что приехал ссылкой. |
