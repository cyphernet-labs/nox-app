...
<!-- SPECKIT START -->
Active feature: **005-onboarding-forms — planned (spec + clarify + plan ready; not yet implemented).** UI-only milestone **M2** of `docs/roadmap.md`: four onboarding/form screens reachable from the Screens gallery — Login/ID entry (2.1), QR scan (2.2), Set username (2.3), Create chat (6.1) — multiplatform (mobile + desktop), all visual states on stubbed data, **backend out of scope**. Plan: `specs/005-onboarding-forms/plan.md` (+ `research.md`, `data-model.md`, `contracts/`, `quickstart.md`). Key decisions: **M2 introduces the first product Freezed BLoCs** — `LoginBloc`/`SetUsernameBloc`/`CreateChatBloc` (value-state + copyWith like `AppRootState`; async form logic ⇒ blueprint 05 §5.1 default, M1 carve-out does not apply), while `QrScanPage` (2.2) stays BLoC-less (`StatefulWidget`, camera stubbed/debug-driven). New shared widgets `AppOnboardCardWidget` (logo+wordmark+hairline; reuse `AppWordmarkWidget`/`AppSplashHairlineWidget`/`AppWindowTitlebarWidget`) + input-field family `AppIdFieldWidget` (mono multiline, Roboto Mono already bundled) / `AppLabeledFieldWidget` (counter + suffix-spinner) + `AppQrOverlayWidget` (the one brand-fixed M2 exception: reticle #FAFAFA + mask #000@55%, per design-system §9.9). No new deps (debounce via `stream_transform`/`rxdart`; camera/QR plugins = Phase 2); add one SVG icon `no_photography`. Availability checks = `const` mock set + ~300ms debounce in-bloc; outcomes via `routeDemo` debug `SegmentedButton`. Full standalone (no inter-screen nav; transitions → M1 `RoutePlaceholderPage`/`AppErrorPage`). Next: `/speckit-tasks`. Prior: **004-splash-simple-screens — planned (M1)**; **003-ui-kit-widgets — implemented**.
<!-- SPECKIT END -->
