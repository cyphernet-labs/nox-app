...
<!-- SPECKIT START -->
Active feature: **004-splash-simple-screens — planned (spec + clarify + plan ready; not yet implemented).** UI-only milestone **M1** of `docs/roadmap.md`: seven standalone screens reachable from the Screens gallery (M0) — Splash (1.1), universal Error (3.1), Appearance (7.3), Language (7.4), Notifications (7.2), Terms (7.6), About (7.7) — multiplatform (mobile + desktop), all visual states on stubbed data, **backend out of scope**. Plan: `specs/004-splash-simple-screens/plan.md` (+ `research.md`, `data-model.md`, `contracts/`, `quickstart.md`). Key decisions: pages per blueprint (`pages/<page>_page/` + `route()` + `Navigator.push`, no router); no new deps (`package_info_plus`/`url_launcher`/`shared_preferences` already present); no Freezed BLoCs for M1 (local `StatefulWidget`/`AppRootBloc` — sanctioned `HomePage`/`UiKitPage` form, BLoCs land with real async); new shared widgets `AppDetailScaffoldWidget`/`AppWindowTitlebarWidget`/`AppThemeOptionWidget`/`AppSettingsSwitchRowWidget`/`AppInfoBannerWidget`; error-screen icons stay SVG `NoxIcons`. Next: `/speckit-tasks`. Prior: **003-ui-kit-widgets — implemented**; **M0 (screens gallery launcher)** done on branch `develop`.
<!-- SPECKIT END -->
