...
<!-- SPECKIT START -->
Active feature: **003-ui-kit-widgets** — build the entire NOX UI kit in `lib/presentation/widgets/` from the `docs/design/system/nox-handoff-2/` catalog, as reusable BLoC-less widgets per blueprint `05`/`06`/`10`, named `App*Widget`. Adapt the reference Dart to token discipline (`ColorScheme`/`context.appColors`, `AppSpacingTokens`/`AppTextStyleTokens`, `NoxRadius`/`NoxElevation`, `NoxBrand`); icons render as **SVG via `NoxIcons`** (no `material_symbols_icons`). Beyond the ~16 custom widgets: full M3 stock-widget theming (`nox_component_themes.dart` → `app_theme.dart`), generic `AppProgressWidget`/`AppErrorWidget`, real SVG illustrations in empty-state, and a dev-only gallery (`lib/main_gallery.dart`). Each widget gets a golden test (curated 2–4 variants × light/dark, local `matchesGoldenFile` harness, `@Tags(['golden'])`, excluded from CI) and a widget test; stock widgets covered by one theme-showcase golden. Plan: `specs/003-ui-kit-widgets/plan.md`. Phase 0/1 artifacts: `research.md`, `data-model.md`, `contracts/ui-kit-api.md`, `quickstart.md`. Spec: `specs/003-ui-kit-widgets/spec.md` (clarified 2026-06-15: `App*Widget` naming; enums without `Nox` prefix → `FileType`/`MessageStatus`/`AppTab`; curated golden set; theme-showcase golden for stock; gallery via separate entrypoint; FR-007 corrected font→SVG).
<!-- SPECKIT END -->
