...
<!-- SPECKIT START -->
Active feature: **003-ui-kit-widgets — implemented (74/74 tasks).** The NOX UI kit lives in `lib/presentation/widgets/` (`chat/`, `primitives/`, `shell/`, `state/`), built from the `docs/design/system/nox-handoff-2/` catalog as reusable BLoC-less `App*Widget` widgets per blueprint `05`/`06`/`10`, with token discipline (`ColorScheme`/`context.appColors`, `AppSpacingTokens`/`AppTextStyleTokens`, `NoxRadius`/`NoxElevation`, `NoxBrand`) and icons as **SVG via `NoxIcons`** (no `material_symbols_icons`). Also done: full M3 stock-widget theming (`nox_component_themes.dart` → `app_theme.dart`), `AppProgressWidget`/`AppErrorWidget`, SVG empty-state illustrations, enums `FileType`/`MessageStatus`/`AppTab` (no `Nox` prefix), and per-widget golden + widget tests (local harness `test/utils/golden.dart`; theme-showcase golden for stock widgets). The dev-only gallery is reached **in-app** via the product launcher (`HomePage` → `UiKitPage.route()`) — the separate `lib/main_gallery.dart` entrypoint was cancelled by the owner (FR-015, 2026-06-15) and does not exist. Spec/plan: `specs/003-ui-kit-widgets/` (note: `spec.md` header still reads `Status: Draft`, stale vs the completed tasks).
<!-- SPECKIT END -->
